...
interface FullSlideOptionsStruct {
	...
	slideStart: (start : number, end : number) => void; 
}
...
class FullSlide {
	...
	element: JQuery;
	options: FullSlideOptionsStruct;
	...
	constructor (element, options) {
		...
		var that = this;
		this.element.bind({
			'fslide-next': function (e : JQueryEventObject) {
				console.log(`fslide-next ${that.element.attr('id')}`);
				that.next(); e.stopPropagation(); },
			'fslide-prev': function (e : JQueryEventObject) {
				that.prev(); e.stopPropagation(); },
			'fslide-to': function (e : JQueryEventObject, idx) {
				console.log(`fslide-to ${idx} ${that.element.attr('id')}`);
				that.slideTo(parseInt(idx)); e.stopPropagation(); }
		});
	...
	slideTo (index : number) : void {
		console.log(`sliding to ${index} ${this.element.attr('id')}`);
